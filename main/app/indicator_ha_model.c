...
static void ha_ctrl_cfg_restore(void) {
	...
	ret = indicator_nvs_read(HA_CFG_STORAGE, (void*)switch_state, &len);

	if(ret == ESP_OK && len == (sizeof(switch_state)))
	{
		ESP_LOGI(TAG, "ctrl cfg read successful");
	}
	else
	{
		// err or not find
...
static void ha_ctrl_cfg_save(void) {
	esp_err_t ret = 0;
	ret = indicator_nvs_write(HA_CFG_STORAGE, (void*)switch_state, sizeof(switch_state));
	if(ret != ESP_OK)
	{
...
static void mqtt_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id, void* event_data) {
	...
	switch((esp_mqtt_event_id_t)event_id)
	{
		...
		default:
			...
			ESP_LOGW(TAG, "If you are always here, please check that your broker is accessible.");
			break;
	}
}
	...
	switch(id)
	{
		case VIEW_EVENT_SENSOR_DATA:
		{
			...
			switch(p_data->sensor_type)
			{
				...
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\":\"%d\"}", CONFIG_SENSOR_BUILDIN_CO2_VALUE_KEY,
								   (int)p_data->value);
					esp_mqtt_client_publish(instance_ptr->mqtt_client, CONFIG_SENSOR_BUILDIN_TOPIC_DATA, data_buf, len,
											0, 0);
					break;
				}
				case SGP40_SENSOR_TVOC:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\":\"%d\"}", CONFIG_SENSOR_BUILDIN_TVOC_VALUE_KEY,
								   (int)p_data->value);
					esp_mqtt_client_publish(instance_ptr->mqtt_client, CONFIG_SENSOR_BUILDIN_TOPIC_DATA, data_buf, len,
											0, 0);
					break;
				...
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\":\"%.1f\"}",
								   CONFIG_SENSOR_BUILDIN_TEMP_VALUE_KEY, p_data->value);
					esp_mqtt_client_publish(instance_ptr->mqtt_client, CONFIG_SENSOR_BUILDIN_TOPIC_DATA, data_buf, len,
											0, 0);
					break;
				}
				...
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\":\"%d\"}",
								   CONFIG_SENSOR_BUILDIN_HUMIDITY_VALUE_KEY, (int)p_data->value);
					esp_mqtt_client_publish(instance_ptr->mqtt_client, CONFIG_SENSOR_BUILDIN_TOPIC_DATA, data_buf, len,
											0, 0);
					break;
				}
		...
		case VIEW_EVENT_HA_SWITCH_ST: /* get the data from user */
		{
			...
			switch(p_data->index)
			{
				case 0:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH1_VALUE_KEY,
								   (int)p_data->value);
				...
				case 1:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH2_VALUE_KEY,
								   (int)p_data->value);
				...
				case 2:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH3_VALUE_KEY,
								   (int)p_data->value);
				...
				case 3:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH4_VALUE_KEY,
								   (int)p_data->value);
				...
				case 4:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH5_VALUE_KEY,
								   (int)p_data->value);
				...
				case 5:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH6_VALUE_KEY,
								   (int)p_data->value);
				...
				case 6:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH7_VALUE_KEY,
								   (int)p_data->value);
				...
				case 7:
				{
					len = snprintf(data_buf, sizeof(data_buf), "{\"%s\": %d}", CONFIG_SWITCH8_VALUE_KEY,
								   (int)p_data->value);
					esp_mqtt_client_publish(instance_ptr->mqtt_client, CONFIG_SWITCH7_TOPIC_STATE, data_buf, len, 0, 0);
					break;
				}
		...
		case VIEW_EVENT_WIFI_ST: /*被动触发*/
		{
			...
			if(p_st->is_network && instance_ptr->is_using)
			{
				esp_event_post_to(mqtt_app_event_handle, MQTT_APP_EVENT_BASE, MQTT_APP_START, &instance_ptr,
								  sizeof(instance_mqtt_t), portMAX_DELAY);
			}
			else
			{
				// shutdown mqtt?
...
static void __cfg_event_handler(void* handler_args, esp_event_base_t base, int32_t id, void* event_data) {
	...
	switch(id)
	{
		case HA_CFG_SET:
			...
			esp_event_post_to(mqtt_app_event_handle, MQTT_APP_EVENT_BASE, MQTT_APP_RESTART, &instance_ptr,
							  sizeof(instance_mqtt_t), portMAX_DELAY);
			break;
		default:
			break;
...
int indicator_ha_model_init(void) {
	...
	/* obtain sensor data */
	ESP_ERROR_CHECK(esp_event_handler_instance_register_with(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_DATA,
